...
const RoundedImageCarousel = ({ slides, autoPlay = true, autoPlayDelay = 5000, hideMainTitle = false, titleBold = true }: RoundedImageCarouselProps) => {
  ...
  return (
    <section id="que-vendemos" className="w-full py-4 bg-background" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Título principal */}
        {!hideMainTitle && (
          <div className="text-center mb-2">
            <h2 className="text-lg sm:text-xl md:text-xl lg:text-2xl font-poppins font-bold text-foreground tracking-wide">
        ...
        <div className="w-full flex flex-col items-center justify-center py-1">
          <div className="px-4 max-w-2xl mb-1 text-center">
            <div
              className={`text-sm sm:text-base md:text-base lg:text-lg xl:text-xl font-poppins ${titleBold ? 'font-bold' : 'font-normal'} tracking-wide text-foreground`}
              style={{ textTransform: "none" }}
            >
              {current.title}
            </div>
            {current.text && (
              <div className="mt-1 text-xs md:text-sm font-poppins font-normal text-foreground no-bold" style={{ textTransform: "none", fontWeight: 400 }}>
                {current.text}
              </div>
            )}
          </div>
              ...
              <div
                ...
                style={{
                  backgroundColor: index === i 
                    ? (i % 2 === 0 ? 'hsl(271 100% 20%)' : 'hsl(45 100% 65%)')
                    : '#D1D5DB'
                }}
              />
            ))}
