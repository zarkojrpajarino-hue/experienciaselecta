...
const RoundedImageCarousel = ({
  ...
  return <section id="que-vendemos" className="w-full py-4 bg-white" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Título del carrusel si existe */}
        {carouselTitle && <div className="text-center mb-6 pt-16">
        ...
        {/* Título principal */}
        {!hideMainTitle && <div className="text-center mb-2">
            <div className="flex items-center justify-center gap-1 sm:gap-2">
              ...
              <TooltipProvider delayDuration={0}>
                ...
                }} onMouseEnter={() => setArrowTooltipOpen(true)} onMouseLeave={() => setArrowTooltipOpen(false)} className="p-0 bg-transparent border-0 cursor-pointer flex-shrink-0" aria-label="Abrir menú">
                      <ChevronUp className="w-4 h-4 sm:w-6 sm:h-6 md:w-7 md:h-7" style={{
                    color: '#D4AF37'
                  }} strokeWidth={3} />
                    </motion.button>
                  </TooltipTrigger>
        ...
        <div className="w-full flex flex-col items-center justify-center py-1 pt-8">
          <div className="px-4 max-w-2xl mb-1 text-center">
            <div className={`text-sm sm:text-base md:text-base lg:text-lg xl:text-xl font-poppins ${titleBold ? 'font-bold' : 'font-normal'} tracking-wide text-black`} style={{
            textTransform: "none"
          }}>
              {current.title}
            </div>
            {current.text && <div className="mt-1 text-xs md:text-sm font-poppins font-normal text-black no-bold" style={{
            textTransform: "none",
            fontWeight: 400
          }}>
                {current.text}
          ...
          <div className="flex gap-2 mb-2">
            {slides.map((_, i) => <div key={i} className="w-2 h-2 rounded-full transition-all duration-300" style={{
            backgroundColor: index === i ? i % 2 === 0 ? '#000000' : '#D4AF37' : '#D1D5DB'
          }} />)}
          </div>
          ...
          <div className="flex gap-3 py-2">
            ...
            scale: 0.9
          }} onClick={() => setIndex(p => (p - 1 + slides.length) % slides.length)} className="p-0 bg-transparent border-0" style={{
            color: '#000000'
          }}>
            ...
            scale: 0.9
          }} onClick={() => setIndex(p => (p + 1) % slides.length)} className="p-0 bg-transparent border-0" style={{
            color: '#000000'
          }}>
        ...
        <AnimatePresence>
          {imageExpanded && <motion.div data-expanded-image initial={{
          height: 0,
          opacity: 0
        }} animate={{
          height: 'auto',
          ...
          ease: 'easeOut'
              ...
              <div className="bg-white border-2 border-[#FFD700]/30 rounded-[2rem] p-4 shadow-xl">
                <div className="flex justify-end mb-2">
                  <Button onClick={toggleImageExpand} className="h-8 w-8 rounded-full bg-white hover:bg-gray-100 text-black shadow-md" size="icon">
                    <X className="h-4 w-4" />
                  </Button>
                </div>
                <img src={current.image} alt={current.alt || (typeof current.title === 'string' ? current.title : 'Imagen ampliada')} className="w-full h-auto object-contain rounded-[1.5rem]" />
              </div>
            </motion.div>}
        </AnimatePresence>

        {/* Image */}
        <div ref={containerRef} data-rounded-carousel-root className={`mx-auto relative flex items-center justify-center rounded-[2rem] overflow-hidden ${isSecondCarousel ? 'w-[60%] sm:w-[50%] md:w-[40%] h-[25vh] md:h-[30vh]' : 'w-full max-w-2xl md:max-w-3xl h-[25vh] md:h-[30vh]'}`} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          {slides.map((s, i) => <motion.div key={i} initial={false} animate={{
          opacity: index === i ? 1 : 0
        }} transition={{
          duration: 0.6
        }} className="absolute inset-0 cursor-pointer rounded-[2rem] overflow-hidden" style={{
